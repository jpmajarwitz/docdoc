...
export default function App({ appShell = 'ai' }) {
  ...
  const zoomZillaStatements = useMemo(() => {
    ...
    return parsedCritiqueJson.statements.map((statement, index) => ({
      id: `statement-${index + 1}`,
      timestamp: statement.timestamp || 'No timestamp provided',
      source: statement.source || 'transcript',
      personName: statement.person_name || statement['person name'] || 'Unknown speaker',
      content: statement.content || ''
    }))
  }, [parsedCritiqueJson])

  const zoomZillaSourceOptions = useMemo(() => {
    return [...new Set(zoomZillaStatements.map((statement) => statement.source).filter(Boolean))]
  }, [zoomZillaStatements])

  const visibleZoomZillaStatements = useMemo(() => {
    if (critiqueCategoryFilter === 'all') return zoomZillaStatements
    return zoomZillaStatements.filter((statement) => statement.source === critiqueCategoryFilter)
  }, [critiqueCategoryFilter, zoomZillaStatements])
                        ...
                        <div className="auth-inline-row">
                          <label>Source
                            <select value={critiqueCategoryFilter} onChange={(event) => setCritiqueCategoryFilter(event.target.value)}>
                              ...
                              {zoomZillaSourceOptions.map((source) => <option key={source} value={source}>{source}</option>)}
                            </select>
                          </label>
                        </div>
                        <div className="doc2deck-slide-grid">
                          {visibleZoomZillaStatements.map((statement, index) => (
                            <div key={statement.id}>
                              <article className="doc2deck-slide-card">
                                <h4>{statement.timestamp} · {statement.personName}</h4>
                                <p><strong>Source:</strong> {statement.source}</p>
                                <p><strong>Content:</strong> {statement.content}</p>
                              </article>
                              {index < visibleZoomZillaStatements.length - 1 ? <hr className="getting-started-item-divider" /> : null}
                            </div>
                          ))}
                        </div>
                      </>
