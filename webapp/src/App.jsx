...
export default function App({ appShell = 'ai' }) {
  ...
  const visibleCritiqueIssues = useMemo(() => {
    ...
    return filtered.sort((left, right) => {
      if (activeView === APP_VIEWS.DOCUMENT_DOCTOR) {
        return extractIssueCardinality(left.id) - extractIssueCardinality(right.id) || `${left.id}`.localeCompare(`${right.id}`)
      }
  ...
  }, [parsedCritiqueJson, critiqueSeverityFilter, critiqueCategoryFilter, critiqueSortMode, activeView])
