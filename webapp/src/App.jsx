...
async function buildPrimaryPromptPreviewText() {
  ...
  if (activeView === APP_VIEWS.REGISTRATION_ADMIN) {
    return (
      <PageShell
        mode={MODES.DOC_DEFINE}
        appTitle="Registration Management"
        appSubtitle="Pending Contact Requests"
        brandFallbackText="Admin"
        topRightControls={
          <>
            {renderBackToSuiteButton()}
            {renderLogoutButton()}
      ...
      >
        <section className="card">
          <div className="auth-toggle-row">
            ...
            <button type="button" className="secondary-button" onClick={() => setAdminTab('create_contact')}>Create Contact</button>
        ...
        {adminTab === 'manage_users' ? (
          <>
            <section className="card">
              <h2>Active Users</h2>
              {!activeUsers.length ? <p>No active users.</p> : activeUsers.map((userRow) => (
                <div key={`active-${userRow.id}`} className="auth-inline-row">
                  <span>{userRow.email}</span>
                  <select value={userRow.account_type || 'subscription'} onChange={(e) => setActiveUsers((rows) => rows.map((r) => (r.id === userRow.id ? { ...r, account_type: e.target.value } : r)))}>
                    <option value="trial">trial</option><option value="subscription">subscription</option>
                  </select>
                  <select value={userRow.status || 'inactive'} onChange={(e) => setActiveUsers((rows) => rows.map((r) => (r.id === userRow.id ? { ...r, status: e.target.value } : r)))}>
                    <option value="active">active</option><option value="inactive">inactive</option>
                  </select>
                  <input type="number" min={1} value={userRow.trial_period_days || ''} onChange={(e) => setActiveUsers((rows) => rows.map((r) => (r.id === userRow.id ? { ...r, trial_period_days: e.target.value } : r)))} />
                  <button type="button" className="header-text-link" onClick={() => handleAdminUserUpdate(userRow)}>Update</button>
                </div>
              ))}
            </section>
            <section className="card">
              <h2>Inactive Users</h2>
              {!inactiveUsers.length ? <p>No inactive users.</p> : inactiveUsers.map((userRow) => (
                <div key={`inactive-${userRow.id}`} className="auth-inline-row">
                  <span>{userRow.email}</span>
                  <select value={userRow.account_type || 'subscription'} onChange={(e) => setInactiveUsers((rows) => rows.map((r) => (r.id === userRow.id ? { ...r, account_type: e.target.value } : r)))}>
                    <option value="trial">trial</option><option value="subscription">subscription</option>
                  </select>
                  <select value={userRow.status || 'inactive'} onChange={(e) => setInactiveUsers((rows) => rows.map((r) => (r.id === userRow.id ? { ...r, status: e.target.value } : r)))}>
                    <option value="active">active</option><option value="inactive">inactive</option>
                  </select>
                  <input type="number" min={1} value={userRow.trial_period_days || ''} onChange={(e) => setInactiveUsers((rows) => rows.map((r) => (r.id === userRow.id ? { ...r, trial_period_days: e.target.value } : r)))} />
                  <button type="button" className="header-text-link" onClick={() => handleAdminUserUpdate(userRow)}>Update</button>
                </div>
              ))}
            </section>
