...
function normalizeRequestError(error) {
  if (error instanceof TypeError) {
    ...
    if (/fetch|network|load failed|failed to load/i.test(message)) {
    ...
    return message || 'A browser TypeError occurred while preparing the request.'
  }
...
async function fetchWithEndpointFallback(endpoint, init) {
  ...
  let lastRetryableGatewayResponse = null
  ...
  for (const candidate of candidates) {
    ...
    if (retryableGatewayStatus) {
      lastRetryableGatewayResponse = response
    }
  ...
  const responseToReturn = lastRetryableGatewayResponse || lastResponse
  if (responseToReturn) {
    responseToReturn.__endpointTrace = endpointTrace
  }
  if (!responseToReturn && lastNetworkError) {
    const traceSummary = summarizeEndpointTrace(endpointTrace)
    throw new Error(
      `Unable to reach the backend API. If this page is loaded over HTTPS, make sure the backend URL is also HTTPS (no mixed content), then rebuild and redeploy your dist files.${traceSummary ? ` Endpoint attempts: ${traceSummary}.` : ''} Last network error: ${lastNetworkError.message || 'fetch failed'}`
    )
  }
...
async function readBackendJson(response) {
  ...
  if (!contentType.includes('application/json')) {
    ...
    if ([502, 503, 504].includes(response.status)) {
      throw new Error(
        `Backend gateway error (status ${response.status}). The backend or hosting gateway timed out or became unavailable before returning JSON.${traceSummary ? ` Endpoint attempts: ${traceSummary}.` : ''} Response preview: ${preview}`
      )
    }
    throw new Error(
      maybeHtml
...
export default function App({ appShell = 'ai' }) {
  ...
  const [zoomChunkingEnabled, setZoomChunkingEnabled] = useState(
    ZOOM_ZILLA_SETTINGS.chunkingEnabledDefault ?? true
  )
  const [zoomChunkSize, setZoomChunkSize] = useState(
    ZOOM_ZILLA_SETTINGS.chunkSizeDefault ?? 40
  )
  ...
  const applyChangesAntiGuidance = isDoc2DeckWorkflow
    ...
    : antiGuidance
  const activeChunkingEnabled = isDoc2DeckWorkflow
    ? doc2DeckChunkingEnabled
    : isZoomZillaWorkflow
      ? zoomChunkingEnabled
      : chunkingEnabled
  const activeChunkSize = isDoc2DeckWorkflow
    ? doc2DeckChunkSize
    : isZoomZillaWorkflow
      ? zoomChunkSize
      : chunkSize
  ...
  const [selectedResunatorOutputTab, setSelectedResunatorOutputTab] = useState('resume')
  const [resunatorRefinedResumeReady, setResunatorRefinedResumeReady] = useState(false)
  const [resunatorGoogleDocSaving, setResunatorGoogleDocSaving] = useState(false)
  const [resunatorGoogleDocLink, setResunatorGoogleDocLink] = useState(null)
  ...
  const orderedCritiqueIssues = useMemo(() => {
    ...
    const combined = isZoomZillaWorkflow
      ...
      ? (parsedCritiqueJson.resume_entries || []).map((item) => ({
        ...
        recommended_change: item.recommended_content,
  ...
  const zoomZillaStatements = useMemo(() => {
    if (!parsedCritiqueJson?.statements) return []
    return parsedCritiqueJson.statements.map((statement, index) => ({
      id: `statement-${index + 1}`,
      source: statement.source || 'transcript',
      personName: (statement.person_name || statement['person name'] || 'Unknown speaker').toUpperCase(),
  ...
  function setChunkSizeForActive(value) {
    ...
    if (isZoomZillaWorkflow) {
      setZoomChunkSize(clampPositiveInteger(value, ZOOM_ZILLA_SETTINGS.chunkSizeDefault ?? 40))
      return
    }
  ...
  function applyUserProfileSettings(settings = {}) {
    ...
    setZoomChunkingEnabled(zoomProfile.chunkingEnabled ?? (ZOOM_ZILLA_SETTINGS.chunkingEnabledDefault ?? true))
    setZoomChunkSize(clampPositiveInteger(zoomProfile.chunkSize, ZOOM_ZILLA_SETTINGS.chunkSizeDefault ?? 40))
  ...
  async function clearApplicationContext(reason = 'Clearing application context.') {
    ...
    setSelectedResunatorOutputTab('resume')
  ...
  function stripResumeMarkdown(value) {
    return String(value || '')
      .replace(/^#{1,6}\s+/, '')
      .replace(/\*\*([^*]+)\*\*/g, '$1')
      .replace(/\*/g, '')
  ...
  function normalizeResumePdfText(value) {
    return String(value || '')
      .normalize('NFKC')
      .replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g, '-')
      .replace(/[\u00ad\u200b-\u200d\ufeff]/g, '')
  ...
  function addPdfTextBlock(doc, rawText, { x, y, maxWidth, lineHeight, bottomMargin, fontStyle = 'normal', fontSize = 10.5 }) {
    doc.setFont('helvetica', fontStyle)
    doc.setFontSize(fontSize)
    if (typeof doc.setCharSpace === 'function') {
      doc.setCharSpace(0)
    }
    const lines = doc.splitTextToSize(normalizeResumePdfText(rawText), maxWidth)
    lines.forEach((line) => {
      if (y > bottomMargin) {
        doc.addPage()
        y = 54
  ...
  function printResunatorMarkdownAsPdf({ content, documentTitle, frameId, emptyError }) {
    if (!String(content || '').trim()) {
      setError(emptyError)
      appendRequestLog('Client-side PDF generation blocked because content was empty.', { documentTitle, frameId })
      return
    }

    appendRequestLog('Preparing RESUnator client-side PDF document.', {
      documentTitle,
      frameId,
      contentLength: String(content || '').length,
      pdfEngine: 'jsPDF',
      note: 'Client-side PDF generation avoids browser print headers and footers such as URL and website name.',
    ...
    try {
      ...
      lines.forEach((line) => {
        ...
        const bullet = trimmed.match(/^[-*•]\s+(.+)$/)
        if (markdownHeading || looksLikeResumeHeader(trimmed)) {
          const headingText = stripResumeMarkdown(markdownHeading ? markdownHeading[2] : trimmed.replace(/[:：]$/, ''))
          y += y === margin ? 0 : 6
          y = addPdfTextBlock(doc, headingText.toUpperCase(), {
            x: margin,
            y,
            maxWidth,
            lineHeight: 15,
            bottomMargin,
            fontStyle: 'bold',
            fontSize: markdownHeading && markdownHeading[1].length === 1 ? 14 : 12
          })
        ...
        const paragraphText = bullet ? `- ${stripResumeMarkdown(bullet[1])}` : stripResumeMarkdown(trimmed)
        y = addPdfTextBlock(doc, paragraphText, {
          x: bullet ? margin + 12 : margin,
          y,
          maxWidth: bullet ? maxWidth - 12 : maxWidth,
          lineHeight: 13,
          bottomMargin,
          fontStyle: 'normal',
          fontSize: 10.5
      ...
      const fileName = `${documentTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'resunator-document'}.pdf`
      appendRequestLog('Saving RESUnator client-side PDF document.', {
        documentTitle,
        frameId,
        fileName,
      ...
      setError(`Unable to generate PDF: ${normalizeRequestError(pdfError)}`)
      appendRequestLog('RESUnator client-side PDF generation failed with an exception.', {
        frameId,
        documentTitle,
        error: normalizeRequestError(pdfError)
      })
    }
  }
  ...
  function saveResunatorResumeAsPdf() {
    printResunatorMarkdownAsPdf({
      ...
      frameId: 'resunator-resume-pdf',
      emptyError: 'There is no generated resume content to save as a PDF.'
    })
  }

  function saveResunatorDisplayedContentAsPdf() {
    const isCoverLetterTab = resunatorCoverLetterMarkdown.trim() && selectedResunatorOutputTab === 'cover_letter'
    printResunatorMarkdownAsPdf({
      content: isCoverLetterTab ? resunatorCoverLetterMarkdown : changedDocumentMarkdown,
      documentTitle: isCoverLetterTab ? 'RESUnator Cover Letter' : 'RESUnator Resume',
      frameId: isCoverLetterTab ? 'resunator-cover-letter-pdf' : 'resunator-resume-pdf',
      emptyError: isCoverLetterTab
        ? 'There is no generated cover letter content to save as a PDF.'
        : 'There is no generated resume content to save as a PDF.'
    })
  }
  ...
  function resunatorMarkdownToWordHtml(content, documentTitle) {
    const htmlParts = []
    let inList = false
    const closeList = () => {
      if (inList) {
    ...
    String(content || '').split(/\r?\n/).forEach((line) => {
      ...
      if (markdownHeading || looksLikeResumeHeader(trimmed)) {
        closeList()
        const headingText = stripResumeMarkdown(markdownHeading ? markdownHeading[2] : trimmed.replace(/[:：]$/, ''))
        htmlParts.push(`<h2>${escapeWordHtml(headingText.toUpperCase())}</h2>`)
        return
      }
      ...
      if (bullet) {
        ...
        htmlParts.push(`<li>${escapeWordHtml(stripResumeMarkdown(bullet[1]))}</li>`)
        return
      }

      closeList()
      htmlParts.push(`<p>${escapeWordHtml(stripResumeMarkdown(trimmed))}</p>`)
    })
    closeList()
...
<head>
<meta charset="utf-8">
<title>${escapeWordHtml(documentTitle)}</title>
<style>
  body { font-family: Aptos, Calibri, Arial, sans-serif; font-size: 11pt; line-height: 1.28; color: #111827; }
  h1 { font-size: 18pt; margin: 0 0 12pt; text-align: center; }
  h2 { font-size: 12pt; margin: 12pt 0 4pt; border-bottom: 1px solid #6b21a8; color: #111827; }
  p { margin: 0 0 6pt; }
...
${htmlParts.join('\n')}
  ...
  function saveResunatorMarkdownAsWord({ content, documentTitle, emptyError }) {
    if (!String(content || '').trim()) {
      setError(emptyError)
      appendRequestLog('Client-side Word generation blocked because content was empty.', { documentTitle })
      return
    }

    try {
      const html = resunatorMarkdownToWordHtml(content, documentTitle)
      const blob = new Blob(['\ufeff', html], { type: 'application/msword;charset=utf-8' })
      const fileName = `${documentTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'resunator-document'}.doc`
      const url = URL.createObjectURL(blob)
      ...
      URL.revokeObjectURL(url)
      appendRequestLog('Saved RESUnator client-side Word document.', {
        documentTitle,
        fileName,
        contentLength: String(content || '').length,
        wordEngine: 'browser Blob HTML document',
        note: 'Client-side Word generation keeps generated content in the browser.'
      })
    } catch (wordError) {
      setError(`Unable to generate Word file: ${normalizeRequestError(wordError)}`)
      appendRequestLog('RESUnator client-side Word generation failed with an exception.', {
        documentTitle,
        error: normalizeRequestError(wordError)
      })
    }
  }

  function saveResunatorDisplayedContentAsWord() {
    const isCoverLetterTab = resunatorCoverLetterMarkdown.trim() && selectedResunatorOutputTab === 'cover_letter'
    saveResunatorMarkdownAsWord({
      content: isCoverLetterTab ? resunatorCoverLetterMarkdown : changedDocumentMarkdown,
      documentTitle: isCoverLetterTab ? 'RESUnator Cover Letter' : 'RESUnator Resume',
      emptyError: isCoverLetterTab
        ? 'There is no generated cover letter content to save as a Word file.'
        : 'There is no generated resume content to save as a Word file.'
    })
  }

  function getResunatorDisplayedDocument() {
    const isCoverLetterTab = resunatorCoverLetterMarkdown.trim() && selectedResunatorOutputTab === 'cover_letter'
    return {
      content: isCoverLetterTab ? resunatorCoverLetterMarkdown : changedDocumentMarkdown,
      documentTitle: isCoverLetterTab ? 'RESUnator Cover Letter' : 'RESUnator Resume',
      emptyError: isCoverLetterTab
        ? 'There is no generated cover letter content to save as a Google Doc.'
        : 'There is no generated resume content to save as a Google Doc.'
    }
  }
  ...
  async function requestGoogleDriveAccessToken() {
    if (!GOOGLE_CLIENT_ID) {
      throw new Error('Google Docs export is not configured. Set VITE_GOOGLE_CLIENT_ID to a Google OAuth web client ID.')
    }

    await loadGoogleIdentityScript()
    if (!window.google?.accounts?.oauth2?.initTokenClient) {
      throw new Error('Google Identity Services is unavailable in this browser session.')
    }
  ...
  function buildGoogleDriveMultipartBody({ metadata, html }) {
    const boundary = `docdoc_google_doc_${Date.now()}_${Math.random().toString(36).slice(2)}`
    const body = new Blob([
      `--${boundary}\r\n`,
  ...
  async function createGoogleDocFromResunatorHtml({ accessToken, html, documentTitle }) {
    const metadata = {
      name: documentTitle,
      mimeType: GOOGLE_DOC_MIME_TYPE
    }
    const { body, contentType } = buildGoogleDriveMultipartBody({ metadata, html })
    ...
    if (!response.ok) {
      const googleError = payload?.error?.message || responseText || `HTTP ${response.status}`
      throw new Error(`Google Drive upload failed: ${googleError}`)
    }
    if (!payload?.id) {
      throw new Error('Google Drive did not return a created file id.')
    }
    return payload
  }

  async function saveResunatorDisplayedContentAsGoogleDoc() {
    const { content, documentTitle, emptyError } = getResunatorDisplayedDocument()
    if (!String(content || '').trim()) {
      setError(emptyError)
      return
    }
    ...
    setError('')
    setResunatorGoogleDocSaving(true)
    setResunatorGoogleDocLink(null)
    appendRequestLog('Starting RESUnator browser-side Google Docs export.', {
      documentTitle,
      scope: GOOGLE_DRIVE_FILE_SCOPE,
      engine: 'Google Drive API multipart conversion upload',
      note: 'Generated content and Google access token stay in the browser.'
    })

    try {
      const accessToken = await requestGoogleDriveAccessToken()
      const html = resunatorMarkdownToWordHtml(content, documentTitle)
      const googleDoc = await createGoogleDocFromResunatorHtml({
        accessToken,
        html,
        documentTitle
      })
      setResunatorGoogleDocLink(googleDoc)
      setStatus(`Created Google Doc: ${googleDoc.name || documentTitle}`)
      appendRequestLog('RESUnator Google Docs export completed.', {
        documentTitle,
        googleFileId: googleDoc.id,
        googleDocName: googleDoc.name,
        webViewLink: googleDoc.webViewLink || null
      })
    } catch (googleDocError) {
      setError(`Unable to create Google Doc: ${normalizeRequestError(googleDocError)}`)
      appendRequestLog('RESUnator Google Docs export failed.', {
        documentTitle,
        error: normalizeRequestError(googleDocError)
      })
    } finally {
      setResunatorGoogleDocSaving(false)
    }
  }
  ...
  function buildLlmRequest(messages) {
    return {
      ...
      deleteFileOnLlm,
      timeoutSeconds: activeSettings.openAiTimeoutSecondsDefault || 180,
...
async function buildPrimaryPromptPreviewText() {
  ...
  function updateResunatorResumeEntry(index, updates) {
    setResunatorResumeEntries((entries) => {
      ...
      if (Object.prototype.hasOwnProperty.call(updates, 'file')) {
        setSelectedResunatorContextResumeSources(
          nextEntries
            .map((entry, entryIndex) => (entry.file ? resunatorResumeSourceLabel(entryIndex) : null))
            .filter(Boolean)
        )
      }
  ...
  function splitTextIntoLineChunks(text, explicitChunkSize) {
    const normalizedChunkSize = Math.max(1, clampPositiveInteger(explicitChunkSize, ZOOM_ZILLA_SETTINGS.chunkSizeDefault ?? 40))
    const lines = `${text || ''}`.split(/\r?\n/)
    if (lines.length === 1 && !lines[0]) {
      return []
    }
    const chunks = []
    for (let index = 0; index < lines.length; index += normalizedChunkSize) {
      const chunkLines = lines.slice(index, index + normalizedChunkSize)
      chunks.push({
        startLine: index + 1,
  ...
  function parseZoomZillaTimestampSeconds(value) {
    const timestampText = `${value || ''}`
    const fullMatch = timestampText.match(/\b(\d{1,2}):(\d{2}):(\d{2})(?:[.,](\d{1,3}))?\b/)
    if (fullMatch) {
      return (
        Number.parseInt(fullMatch[1], 10) * 3600 +
        Number.parseInt(fullMatch[2], 10) * 60 +
        Number.parseInt(fullMatch[3], 10) +
        Number.parseInt((fullMatch[4] || '0').padEnd(3, '0'), 10) / 1000
      )
    }
    const shortMatch = timestampText.match(/\b(\d{1,2}):(\d{2})(?:[.,](\d{1,3}))?\b/)
    if (!shortMatch) {
      return null
    }
    return (
      Number.parseInt(shortMatch[1], 10) * 60 +
      Number.parseInt(shortMatch[2], 10) +
      Number.parseInt((shortMatch[3] || '0').padEnd(3, '0'), 10) / 1000
    )
  }

  function parseZoomZillaTimestampsSeconds(value) {
    const timestampText = `${value || ''}`
    const matches = []
    const fullPattern = /\b(\d{1,2}):(\d{2}):(\d{2})(?:[.,](\d{1,3}))?\b/g
    let match = fullPattern.exec(timestampText)
    while (match) {
      matches.push(
        Number.parseInt(match[1], 10) * 3600 +
          Number.parseInt(match[2], 10) * 60 +
          Number.parseInt(match[3], 10) +
          Number.parseInt((match[4] || '0').padEnd(3, '0'), 10) / 1000
      )
      match = fullPattern.exec(timestampText)
    }
    if (matches.length) {
      return matches
    }

    const shortPattern = /\b(\d{1,2}):(\d{2})(?:[.,](\d{1,3}))?\b/g
    match = shortPattern.exec(timestampText)
    while (match) {
      matches.push(
        Number.parseInt(match[1], 10) * 60 +
          Number.parseInt(match[2], 10) +
          Number.parseInt((match[3] || '0').padEnd(3, '0'), 10) / 1000
      )
      match = shortPattern.exec(timestampText)
    }
    return matches
  ...
  function formatZoomZillaTimestamp(seconds) {
  ...
  function buildZoomZillaChatChunkForVttRange(chatText, range) {
    ...
    String(chatText || '').split(/\r?\n/).forEach((line, index) => {
      ...
      if (Number.isFinite(activeTimestamp) && activeTimestamp >= normalizedStart && activeTimestamp <= normalizedEnd) {
  ...
  function buildZoomZillaChunkCritiqueRequest({ vttChunk, chatChunk, chunkIndex, chunkCount }) {
    const chunkLabel = `chunk ${chunkIndex + 1} of ${chunkCount}`
    const messages = [
      {
        type: 'input_text',
        text: [
          `Review this Zoom-Zilla transcript ${chunkLabel} asynchronously as part of a larger meeting transcript.`,
          `Objective: ${objective}`,
          `Anti-Guidance: ${buildAntiGuidancePrompt()}`,
          'Process only the chunk content provided in this request. Preserve statement order within the chunk.',
          'Use source "transcript" for VTT spoken content and source "chat" for chat transcript content.'
        ].join('\n')
      }
    ]
    ...
    if (vttChunk) {
      messages.push({
        type: 'input_text',
        text: `VTT TRANSCRIPT ${chunkLabel.toUpperCase()} LINES ${vttChunk.startLine}-${vttChunk.endLine}:\n${vttChunk.text}`
      })
    }
    ...
    if (chatChunk) {
      messages.push({
        type: 'input_text',
        text: `CHAT TRANSCRIPT ${chunkLabel.toUpperCase()} FILTERED TO VTT TIME RANGE ${formatZoomZillaTimestamp(chatChunk.rangeStart) || 'unknown'}-${formatZoomZillaTimestamp(chatChunk.rangeEnd) || 'unknown'}${chatChunk.startLine ? ` (CHAT LINES ${chatChunk.startLine}-${chatChunk.endLine})` : ''}:\n${chatChunk.text || '_No chat lines fell within this VTT chunk time range._'}`
      })
    }
  ...
  function buildResunatorCritiqueRequest() {
    const messages = [
      {
        type: 'input_text',
        text: `Critique the attached Resume against the attached Job Description. Objective: ${objective} Anti-Guidance: ${buildAntiGuidancePrompt()}`
      },
  ...
  function formatResunatorJobFitAnalysisForPrompt() {
    const entries = parsedCritiqueJson?.job_fit_analysis || []
    if (!entries.length) return ''
    return entries.map((entry, index) => {
      return [
        `Job Fit Analysis ${index + 1}`,
        `Overall Fit: ${entry.overall_fit}`,
        `Strengths: ${entry.strengths}`,
        `Gaps and Mitigations: ${entry.gaps_and_mitigations}`,
        `Other Factors: ${entry.other_factors}`
      ].join('\n')
    }).join('\n\n')
  }
  ...
  function buildApplyChangeItemsRequest(changeItemsInput = changeItems) {
    ...
    const applyMessages = [
      ...
      ...(isResunatorWorkflow && formatResunatorJobFitAnalysisForPrompt()
        ? [
            {
              type: 'input_text',
              text: `Job Fit Analysis:\n${formatResunatorJobFitAnalysisForPrompt()}`
            }
          ]
        : []),
      ...(!isResunatorWorkflow
  ...
  function buildResunatorCoverLetterRequest() {
    const messages = [
      ...
      {
        type: 'input_text',
        text: `Create or refine a curated cover letter that aligns this generated resume with the job description. Generated resume content:
${changedDocumentMarkdown}`
      },
      ...(resunatorCoverLetterMarkdown.trim()
        ? [
            {
              type: 'input_text',
              text: `Existing cover letter to refine:
${resunatorCoverLetterMarkdown}`
            }
          ]
        : []),
      {
...
${formatChangeItems(changeItems)}`
  ...
  async function refineResunatorResume() {
    ...
    setLoading(true)
    setLastOperation(OPERATIONS.APPLY_CHANGE_ITEMS)
    setResunatorRefinedResumeReady(false)
    clearRequestLog()
    ...
    try {
      ...
      setResunatorRefinedResumeReady(true)
  ...
  async function generateResunatorCoverLetter() {
    ...
    try {
      ...
      setSelectedResunatorOutputTab('cover_letter')
      setStatus(resunatorCoverLetterMarkdown.trim() ? 'Cover letter refined successfully.' : 'Cover letter generated successfully.')
    ...
    const messages = [
      {
        ...
        text: isResunatorWorkflow
          ? `Critique the included changed ${contentNoun} using the original review configuration. Objective: ${objective} Anti-Guidance: ${buildAntiGuidancePrompt()}`
  ...
  function validateCritiqueJsonOutput(outputText) {
    ...
    if (isResunatorWorkflow) {
      ...
      parsed.resume_entries = resumeEntries.map((item) => {
        if (!item || typeof item !== 'object') throw new Error('resume entry must be an object.')
        const recommendedContent = item['recommended content'] ?? item.recommended_content
        ;['id', 'category', 'explanation', 'confidence_notes'].forEach((field) => {
          if (typeof item[field] !== 'string' || !item[field].trim()) throw new Error(`resume entry ${field} must be a non-empty string.`)
        })
        if (typeof recommendedContent !== 'string' || !recommendedContent.trim()) throw new Error('resume entry recommended content must be a non-empty string.')
        return {
          ...item,
      ...
      const llmData =
        operation === OPERATIONS.CRITIQUE_PRIMARY
          ? await (async () => {
              ...
              const runSinglePrimaryRequest = async (requestPayload, chunkContext = null, fileEntriesOverride = null) => {
                const requestDirectFileEntries = fileEntriesOverride ?? directFileEntries
                ...
                appendRequestLog('Submitting primary critique request to backend.', {
                  ...
                  chunkContext,
                  fileEntries: Object.fromEntries(
                    Object.entries(requestDirectFileEntries).map(([key, file]) => [
                      key,
                      file ? { name: file.name, size: file.size, type: file.type } : null
                    ])
                  )
              ...
              const shouldUseZoomZillaFileChunks = isZoomZillaWorkflow && activeChunkingEnabled
              if (shouldUseZoomZillaFileChunks) {
                const normalizedChunkSize = clampPositiveInteger(activeChunkSize, ZOOM_ZILLA_SETTINGS.chunkSizeDefault ?? 40)
                const [vttText, chatText] = await Promise.all([
                  readFileAsTextPayload(zoomVttFile),
                  readFileAsTextPayload(zoomChatFile)
                ])
                const vttChunks = splitTextIntoLineChunks(vttText, normalizedChunkSize)
                const chunkCount = vttChunks.length
                if (!chunkCount) {
                  throw new Error('Select a VTT transcript with text content before generating a transcript.')
                }

                appendRequestLog('Zoom-Zilla file chunking plan calculated.', {
                  chunkSize: normalizedChunkSize,
                  chunkCount,
                  vttChunkCount: vttChunks.length,
                  chatFiltering: 'Chat lines are selected per chunk by matching chat timestamps to each VTT chunk time range.',
                  vttLineCount: vttText ? vttText.split(/\r?\n/).length : 0,
                  chatLineCount: chatText ? chatText.split(/\r?\n/).length : 0
                }, { submissionId })
                ...
                for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex += 1) {
                  const vttChunk = vttChunks[chunkIndex] || null
                  appendRequestLog('Preparing Zoom-Zilla file chunk request.', {
                    chunkIndex: chunkIndex + 1,
                    chunkCount,
                    chunkSize: normalizedChunkSize,
                    vttLines: vttChunk ? `${vttChunk.startLine}-${vttChunk.endLine}` : null
                  }, { submissionId })
                  const vttTimeRange = extractZoomZillaTimestampRange(vttChunk?.text || '')
                  const chatChunk = buildZoomZillaChatChunkForVttRange(chatText, vttTimeRange)
                  setStatus(`Invoking ${operationLabels[operation]} file chunk ${chunkIndex + 1} of ${chunkCount}...`)
                  appendRequestLog('Invoking Zoom-Zilla file chunk request.', {
                    chunkIndex: chunkIndex + 1,
                    chunkCount,
                    chunkSize: normalizedChunkSize,
                    vttLines: vttChunk ? `${vttChunk.startLine}-${vttChunk.endLine}` : null,
                    vttTimeStart: formatZoomZillaTimestamp(vttTimeRange.start),
                    vttTimeEnd: formatZoomZillaTimestamp(vttTimeRange.end),
                    chatLines: chatChunk.startLine ? `${chatChunk.startLine}-${chatChunk.endLine}` : null,
                    chatLineCount: chatChunk.lineCount
                  }, { submissionId })

                  const chunkResponse = await runSinglePrimaryRequest(
                    buildZoomZillaChunkCritiqueRequest({ vttChunk, chatChunk, chunkIndex, chunkCount }),
                    {
                      chunkIndex: chunkIndex + 1,
                      chunkCount,
                      chunkSize: normalizedChunkSize,
                  ...
                  responseSize += (chunkResponse.outputText || '').length
                  const parsedChunk = validateCritiqueJsonOutput(chunkResponse.outputText)
                  mergedStatements.push(...(parsedChunk.statements || []))
                  setStatus(`Completed ${chunkIndex + 1}/${chunkCount} file chunks...`)
                }
                ...
                const mergedOutput = {
                  ...
                  metadata: {
                    ...
                    input_digest: `zoom-zilla-chunked-${chunkCount}-${normalizedChunkSize}`,
                    prompt_size: vttText.length + chatText.length,
                    document_size: vttText.length + chatText.length,
                    response_size: responseSize,
                    processing_time_ms: Math.max(0, Date.now() - startedAt)
                  }
                }
                appendRequestLog('Zoom-Zilla file chunks merged into integrated transcript JSON.', {
                  chunkCount,
                  statementCount: mergedStatements.length,
                  responseSize
                }, { submissionId })
                return { outputText: JSON.stringify(mergedOutput, null, 2), chunked: true }
              }
  ...
  useEffect(() => {
    ...
    const settings = {
      ...
      zoomZilla: {
        ...
        chunkingEnabled: zoomChunkingEnabled,
        chunkSize: zoomChunkSize,
    ...
    function renderSettingsField(settingKey) {
      switch (settingKey) {
        ...
        case 'chunkingEnabled':
          if (!isDeckMateWorkflow && !isZoomZillaWorkflow) {
            return null
          }
          return (
            <label key={settingKey} className="checkbox-label">
        ...
        case 'chunkSize':
          if (!isDeckMateWorkflow && !isDoc2DeckWorkflow && !isZoomZillaWorkflow) {
            return null
          }
          return (
            <label key={settingKey}>
              ...
              {isResunatorWorkflow ? (
                <>
                  ...
                  <div className="field-group">
                    <div className="auth-inline-row resunator-job-req-options">
                      {['file', 'text', 'url'].map((mode) => (
                    ...
                    {jobReqInputMode === 'text' ? (
                      <label>
                        Job Req Text
                        <textarea
                          className="resunator-job-req-textarea"
                          name="job_req_text"
                          value={jobReqText}
                  ...
                  <div className="field-group resunator-resume-version-list">
                    {resunatorResumeEntries.map((entry, index) => (
                      <div key={index} className="resunator-resume-entry">
                        <label className="panel-label resunator-resume-source-label">{resunatorResumeSourceLabel(index)}</label>
                        <div className="file-selector-row">
                          <input
                          ...
                          {resunatorResumeEntries.length > 1 ? (
                            <button type="button" className="secondary-button compact-resunator-button" onClick={() => removeResunatorResumeEntry(index)}>
                              Remove
                            </button>
                          ) : null}
                        </div>
                    ...
                    <button type="button" className="secondary-button compact-resunator-button" onClick={addResunatorResumeEntry}>
                      Add Resume Version
                    </button>
          ...
          <div className="action-row wrap-actions center-actions">
            {lastOperation === OPERATIONS.APPLY_CHANGE_ITEMS ? (
              <button type="button" onClick={() => setCurrentMode(MODES.VIEW_CHANGED)}>
                {isResunatorWorkflow && resunatorRefinedResumeReady ? 'View Refined Resume' : `View Changed ${contentNounTitle}`}
              </button>
            ) : (
            ...
            <div className="change-composer-card">
              {isDeckMateWorkflow ? (
                <>
                  ...
                  <button type="button" className="compact-change-button" onClick={addDeckIssueSelectionsAsChangeItems}>
                    ...
                    >
                      {visibleCritiqueIssues.map((issue) => (
                        <option key={issue.id} value={issue.id}>
                          {isResunatorWorkflow ? `${issue.id} · ${issue.category}` : `${issue.id} · ${issue.category} · ${issue.severity}`}
                        </option>
                      ))}
                    </select>
                  </label>
                  <button type="button" className="compact-change-button" onClick={addDocIssueSelectionsAsChangeItems}>
                    Add Selected Issues as Change Items
                  </button>
                  {!isResunatorWorkflow ? (
                    <>
                    ...
                    activeResunatorResumeEntries().map((entry) => {
                      ...
                      return (
                        <label key={source} className="resunator-context-choice">
                          ...
                          />
                          <span className="resunator-context-source">{source}</span>
                          <span className="resunator-context-file">
  ...
  return (
    ...
    >
      <section className="card action-row wrap-actions center-actions compact-panel">
        {isResunatorWorkflow ? (
          <>
            <button type="button" className="secondary-button" onClick={saveResunatorDisplayedContentAsPdf}>
              Generate PDF
            </button>
            <button type="button" className="secondary-button" onClick={saveResunatorDisplayedContentAsWord}>
              Save as Word
            </button>
            <button
              type="button"
              className="secondary-button"
              onClick={saveResunatorDisplayedContentAsGoogleDoc}
              disabled={resunatorGoogleDocSaving}
            >
              {resunatorGoogleDocSaving ? 'Saving to Google Docs...' : 'Save to Google Docs'}
            </button>
            {resunatorGoogleDocLink?.webViewLink ? (
              <a
                className="secondary-button link-button"
                href={resunatorGoogleDocLink.webViewLink}
                target="_blank"
                rel="noreferrer"
              >
                Open Google Doc
      ...
      {isResunatorWorkflow ? (
        <section className="card field-group compact-panel resunator-instruction-panel">
          <div className="resunator-instruction-row">
            <button
              type="button"
          ...
          <div className="resunator-instruction-row">
            ...
            >
              {resunatorCoverLetterMarkdown.trim() ? 'Refine Cover Letter' : 'Generate Cover Letter'}
            </button>
            <label>
      ...
      <section className="card field-group tall-document-panel">
        {isDeckMateWorkflow ? (
          ...
          <>
            {resunatorCoverLetterMarkdown.trim() ? (
              <div className="settings-tabs" role="tablist" aria-label="RESUnator generated content tabs">
                <button
                  type="button"
                  className={selectedResunatorOutputTab === 'resume' ? 'settings-tab active' : 'settings-tab'}
                  onClick={() => setSelectedResunatorOutputTab('resume')}
                >
                  Resume
                </button>
                <button
                  type="button"
                  className={selectedResunatorOutputTab === 'cover_letter' ? 'settings-tab active' : 'settings-tab'}
                  onClick={() => setSelectedResunatorOutputTab('cover_letter')}
                >
                  Cover Letter
                </button>
              </div>
            ) : null}
            {selectedResunatorOutputTab === 'cover_letter' && resunatorCoverLetterMarkdown.trim() ? (
              <label>
