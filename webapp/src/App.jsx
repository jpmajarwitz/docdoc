...
async function buildPrimaryPromptPreviewText() {
  ...
  function buildZoomZillaChunkCritiqueRequest({ vttChunk, chatChunk, chunkIndex, chunkCount }) {
    ...
    if (chatChunk) {
      messages.push({
        type: 'input_text',
        text: `CHAT TRANSCRIPT ${chunkLabel.toUpperCase()} LINES ${chatChunk.startLine}-${chatChunk.endLine}:\n${chatChunk.text}`
      })
    }
        ...
        operation === OPERATIONS.CRITIQUE_PRIMARY
          ? await (async () => {
              ...
              if (shouldUseZoomZillaFileChunks) {
                ...
                const chatChunks = splitTextIntoLineChunks(chatText, normalizedChunkSize)
                const chunkCount = Math.max(vttChunks.length, chatChunks.length)
                if (!chunkCount) {
                  throw new Error('Select a VTT transcript or chat transcript with text content before generating a transcript.')
                }
                ...
                for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex += 1) {
                  ...
                  const chatChunk = chatChunks[chunkIndex] || null
                  ...
                  appendRequestLog('Invoking Zoom-Zilla file chunk request.', {
                    ...
                    chatLines: chatChunk ? `${chatChunk.startLine}-${chatChunk.endLine}` : null
                  }, { submissionId })
