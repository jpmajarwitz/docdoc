...
function php_backend_apply_cors($config)
{
    ...
    $allowedOrigins = array_merge($allowedOrigins, array_values(array_filter([
...
function php_backend_validate_llm_request($payload)
{
    ...
    return [
        ...
        'timeoutSeconds' => php_backend_clamp_timeout_seconds(isset($payload['timeoutSeconds']) ? $payload['timeoutSeconds'] : 180),
    ];
}

function php_backend_clamp_timeout_seconds($timeoutSeconds)
{
...
function php_backend_curl_request($url, $headers, $payload = '', $method = 'POST', $timeoutSeconds = 180)
{
    ...
    $options = [
        ...
        CURLOPT_TIMEOUT => php_backend_clamp_timeout_seconds($timeoutSeconds),
    ];
...
function php_backend_upload_file_to_openai($apiKey, $upload, $timeoutSeconds = 180)
{
...
function php_backend_build_input_content($llmRequest, $fileMap, $apiKey)
{
    ...
    foreach ($llmRequest['messages'] as $message) {
        ...
        $fileId = php_backend_upload_file_to_openai($apiKey, $fileMap[$source], $llmRequest['timeoutSeconds'] ?? 180);
        if (!$fileId) {
            php_backend_error(400, "Missing uploaded file for source '{$source}'.");
        }
...
function php_backend_invoke_llm($llmRequest, $fileMap, $config)
{
    ...
    php_backend_log('llm_invoke_start', [
        ...
        'timeoutSeconds' => $llmRequest['timeoutSeconds'] ?? 180,
    ]);
