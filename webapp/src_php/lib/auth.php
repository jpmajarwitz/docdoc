...
{
    $stmt = $pdo->prepare('SELECT id, email, display_name, status, account_type, trial_period_days, trial_start_at, user_type FROM users WHERE id = :id LIMIT 1');
...
function auth_ensure_user_trial_columns(PDO $pdo)
{
    ...
    if (!isset($columns['user_type'])) {
        $alterFragments[] = "ADD COLUMN user_type VARCHAR(40) NOT NULL DEFAULT 'regular'";
    }
...
function auth_contact_columns(PDO $pdo)
{
    static $cached = null;
    if (is_array($cached)) {
        return $cached;
...
function auth_ensure_contact_columns(PDO $pdo)
{
    try {
        $columns = auth_contact_columns($pdo);
    } catch (Throwable $e) {
        return;
    }
    ...
    if (!isset($columns['access'])) {
        $alterFragments[] = "ADD COLUMN access VARCHAR(20) NOT NULL DEFAULT 'pending'";
    }
...
function auth_get_contact_by_email(PDO $pdo, $emailNormalized)
{
    try {
        $stmt = $pdo->prepare('SELECT * FROM contact WHERE email_normalized = :email LIMIT 1');
        $stmt->execute(['email' => $emailNormalized]);
        return $stmt->fetch() ?: null;
    } catch (Throwable $e) {
        return null;
...
function auth_upsert_contact_request(PDO $pdo, $payload)
{
    $stmt = $pdo->prepare(
        'INSERT INTO contact (first_name, last_name, phone_number, email, email_normalized, job_title, `access`, duration, created_at, updated_at)
         VALUES (:first_name, :last_name, :phone_number, :email, :email_normalized, :job_title, :access, :duration, UTC_TIMESTAMP(), UTC_TIMESTAMP())
