...
$rows = $stmt->fetchAll() ?: [];
$active = array_values(array_filter($rows, fn($r) => strtolower((string)($r['status'] ?? '')) === 'active'));
$inactive = array_values(array_filter($rows, fn($r) => strtolower((string)($r['status'] ?? '')) !== 'active'));
