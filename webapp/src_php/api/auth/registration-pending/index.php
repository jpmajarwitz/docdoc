...
$pendingStmt = $pdo->query("SELECT id, first_name, last_name, phone_number, email, job_title, `access`, created_at, updated_at FROM contact WHERE LOWER(`access`) = 'pending' ORDER BY created_at DESC");
$pendingRows = $pendingStmt->fetchAll() ?: [];
$approvedStmt = $pdo->query("SELECT id, first_name, last_name, phone_number, email, job_title, `access`, created_at, updated_at FROM contact WHERE LOWER(`access`) = 'approved' ORDER BY updated_at DESC");
$approvedRows = $approvedStmt->fetchAll() ?: [];

php_backend_json_response(200, [
    'ok' => true,
    'items' => $pendingRows,
    'approvedItems' => $approvedRows,
]);
