...
class LlmRequest(BaseModel):
    ...
    timeout_seconds: int = Field(180, alias='timeoutSeconds')
...
def clamp_timeout_seconds(timeout_seconds: int | None) -> int:
    if timeout_seconds is None:
        return 180
    return max(30, min(int(timeout_seconds), 900))


def get_client(timeout_seconds: int | None = None) -> OpenAI:
    api_key = os.getenv('OPENAI_API_KEY')
    if not api_key:
        raise HTTPException(status_code=500, detail='OPENAI_API_KEY is not configured on the backend.')
    return OpenAI(api_key=api_key, timeout=clamp_timeout_seconds(timeout_seconds))
...
async def invoke_llm(llm_request: LlmRequest, file_map: dict[str, UploadFile | None]) -> LlmResponse:
    client = get_client(llm_request.timeout_seconds)
...
async def apply_change_items(http_request: Request) -> LlmResponse:
    form = await http_request.form()
    request_payload = form.get('request')
    if not isinstance(request_payload, str):
        raise HTTPException(status_code=400, detail='Missing request form field.')
